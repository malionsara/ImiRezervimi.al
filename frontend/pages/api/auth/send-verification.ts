// frontend/pages/api/auth/send-verification.ts
// API endpoint for sending SMS verification codes
// Albanian Beauty Salon Booking Platform

import { NextApiRequest, NextApiResponse } from 'next';
import { sendVerificationSMS, cleanupExpiredCodes } from '../../../lib/sms';
...
export default async function handler(
  ...
  try {
    ...
    const { phone }: SendVerificationRequest = req.body;
    ...
    console.log(`SMS verification request from ${xForwardedFor} for ${phone}`);

    // Send verification SMS
    const result = await sendVerificationSMS(phone);

    if (!result.success) {
      // Rate limit or other SMS error
      const statusCode = result.error?.includes('prisni') ? 429 : 500;
      
      return res.status(statusCode).json({
        success: false,
        error: {
          code: statusCode === 429 ? 'RATE_LIMITED' : 'SMS_ERROR',
          message: result.error || 'Gabim në dërgimin e SMS-it',
        },
      });
    }
    ...
    return res.status(500).json({
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: 'Gabim në dërgimin e kodit të verifikimit',
