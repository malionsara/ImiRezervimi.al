// frontend/pages/api/auth/verify-phone.ts
// API endpoint for verifying phone numbers with SMS codes
// Albanian Beauty Salon Booking Platform

import { NextApiRequest, NextApiResponse } from 'next';
import { verifyPhoneCode, sendWelcomeSMS } from '../../../lib/sms';
...
export default async function handler(
  ...
  try {
    const { phone, code }: VerifyPhoneRequest = req.body;
    ...
    console.log(`Phone verification attempt from ${xForwardedFor} for ${phone}`);
    ...
    if (!result.success) {
      ...
      return res.status(statusCode).json({
        ...
        error: {
          ...
          message: result.error || 'Verifikimi i kodit dështoi',
        },
      });
    }
    ...
    if (result.verified) {
      // Send welcome SMS asynchronously (don't wait for it)
      sendWelcomeSMS(phone).catch(console.error);

      console.log(`Phone ${phone} successfully verified`);
    }
